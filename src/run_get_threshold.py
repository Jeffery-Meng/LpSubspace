...
internal_file = root_file / "result2"
count = 0
...
for lp, lp_dict in repetitions.items():
    ...
    for dataset, exp_list in lp_dict.items():
        ...
        for rank, solution, proj_dim, bigl, threshold in exp_list:
            ...
            conf["script"][1] = "ROOT/src/get_threshold.py"
            conf["subspace rank"] = rank
            conf["projection dimension"] = proj_dim
            conf["norm p"] = lp
            conf["k"] = 10
            conf["solution"] = solution
            conf["recall"] = np.linspace(0.85, 0.95, 11).tolist()
            ...
            json_file = str(
                config_auto_path / "threshold_GEN_{}.json".format(count)
            )
